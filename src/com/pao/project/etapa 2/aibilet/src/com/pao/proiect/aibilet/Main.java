...
public class Main {
    ...
    private void registerOrganizerFlow() {
        ...
        try {
            ...
            audit("inregistrare_organizator");
        } catch (IllegalArgumentException e) {
            System.out.println("Eroare: " + e.getMessage());
        }
    }
    ...
    private void adminMenu(Utilizator admin) {
        while (true) {
            ...
            System.out.println("10. Raport vanzari pe evenimente");
            System.out.println("0. Logout");
    ...
    private void listAllEvents() {
        ...
        audit("afiseaza_toate_evenimentele");
    ...
    private void searchEventsFlow() {
        ...
        audit("cauta_eveniment_dupa_titlu");
    ...
    private void filterEventsByCityFlow() {
        ...
        audit("filtreaza_evenimente_dupa_oras");
    ...
    private void filterEventsByCategoryFlow() {
        ...
        Eveniment[] toate = evenimentService.listAll();
        audit("filtreaza_evenimente_dupa_categorie");
    ...
    private void listClientTickets(Client client) {
        List<BiletClientView> bilete = ticketingService.getBileteClientCuEvenimentSiLocatie(client.getId());
        audit("afiseaza_biletele_mele");
        ...
        for (int i = 0; i < bilete.size(); i++) {
            afiseazaBiletClientView(bilete.get(i));
        }
    }

    private void listClientOrders(Client client) {
        List<ComandaClientView> comenzi = ticketingService.getComenziClientCuNumarBilete(client.getId());
        audit("afiseaza_comenzile_mele");
        ...
        for (int i = 0; i < comenzi.size(); i++) {
            afiseazaComandaClientView(comenzi.get(i));
        }
    }

    private void listEvenimentVanzariReport() {
        List<EvenimentVanzariView> raport = evenimentService.listEvenimenteCuNumarBileteVandute();
        audit("raport_vanzari_evenimente");

        if (raport.isEmpty()) {
            System.out.println("Nu exista evenimente pentru raport.");
            return;
        }

        for (int i = 0; i < raport.size(); i++) {
            afiseazaEvenimentVanzariView(raport.get(i));
        }
    }
    ...
    private void publishSeatedEventFlow(Organizator organizator) {
        try {
            ...
            while (true) {
                String tip = citesteTextObligatoriu("Nume tip bilet: ", "Tipul de bilet este obligatoriu.");
                double pret = citesteDouble("Pret pentru " + tip + ": ");
                boolean aplicat = false;

                while (!aplicat) {
                    String pozitii = citesteTextObligatoriu(
                            "Unde se aplica tipul (coduri separate prin spatiu/virgula, interval ex: A1-B2, TOATE sau RESTUL): ",
                            "Trebuie specificate pozitiile pentru tipul de bilet.");
            ...
            audit("adauga_eveniment_cu_locuri");
    ...
    private void publishStandingEventFlow(Organizator organizator) {
        try {
            ...
            for (int i = 0; i < numarTipuri; i++) {
                ...
                String nume = citesteTextObligatoriu("Nume tip: ", "Tipul de bilet este obligatoriu.");
            ...
            System.out.println("Eveniment creat cu succes.");
            afiseazaEveniment(eveniment);
            audit("adauga_eveniment_fara_locuri");
    ...
    private void deleteOrganizerEventFlow(Organizator organizator) {
        ...
        try {
            ...
            evenimentService.delete(eventId);
            System.out.println("Eveniment sters.");
            audit("sterge_eveniment");
        } catch (EntitateInexistentaException e) {
            System.out.println("Eroare: " + e.getMessage());
        } catch (OperatieNepermisaException e) {
            System.out.println("Eroare: " + e.getMessage());
        ...
        try {
            ...
            if (eveniment instanceof EvenimentCuLocuri) {
                ...
                if (citesteBoolean("Doresti sa aduci modificari hartii de locuri? (da/nu): ")) {
                    ...
                    while (schimbaTipLoc) {
                        String tip = citesteTextObligatoriu("Nume tip bilet (ex: VIP, STANDARD): ",
                                "Tipul de bilet este obligatoriu.");
                        String pozitii = citesteTextObligatoriu("Coduri locuri (ex: A1-B2, C4): ",
                                "Trebuie specificate pozitiile pentru tipul de bilet.");
                        try {
    ...
    private void deleteUserFlow(Utilizator admin) {
        ...
        try {
            ...
            utilizatorService.delete(userId);
            System.out.println("Utilizator sters.");
            audit("sterge_utilizator");
    ...
    private void removeAgentCheckInFromEventFlow() {
        ...
        try {
            ...
            audit("sterge_asignare_agent_checkin");
        } catch (EntitateInexistentaException e) {
            System.out.println("Eroare: " + e.getMessage());
        } catch (IllegalArgumentException e) {
            System.out.println("Eroare: " + e.getMessage());
    ...
    private void listAssignedEvents(AgentCheckIn agent) {
        ...
        audit("afiseaza_evenimente_asignate_agent");
    ...
    private void afiseazaBiletClientView(BiletClientView bilet) {
        afiseazaTitluMic("Bilet");
        System.out.println("ID bilet: " + bilet.getBiletId());
        System.out.println("Cod: " + bilet.getCodBilet());
        System.out.println("Status: " + bilet.getStatusBilet());
        System.out.println("Pret: " + bilet.getPret() + " RON");
        System.out.println("Eveniment: " + bilet.getTitluEveniment());
        System.out.println("Locatie: " + bilet.getNumeLocatie() + ", " + bilet.getOras());
        if (bilet.getCodLoc() == null) {
            System.out.println("Loc: fara loc alocat");
        } else {
            System.out.println("Loc: " + bilet.getCodLoc() + " (rand=" + bilet.getRand() + ", coloana=" + bilet.getColoana() + ")");
        }
    }

    private void afiseazaComandaClientView(ComandaClientView comanda) {
        afiseazaTitluMic("Comanda");
        System.out.println("ID comanda: " + comanda.getComandaId());
        System.out.println("Data: " + comanda.getDataComanda());
        System.out.println("Total: " + comanda.getTotal() + " RON");
        System.out.println("Numar bilete: " + comanda.getNumarBilete());
    }

    private void afiseazaEvenimentVanzariView(EvenimentVanzariView eveniment) {
        afiseazaTitluMic("Raport vanzari eveniment");
        System.out.println("ID eveniment: " + eveniment.getEvenimentId());
        System.out.println("Titlu: " + eveniment.getTitluEveniment());
        System.out.println("Bilete vandute: " + eveniment.getNumarBileteVandute());
        System.out.println("Venit total: " + eveniment.getVenitTotal() + " RON");
    }
    ...
    private String citesteTextObligatoriu(String mesaj, String mesajEroare) {
        while (true) {
