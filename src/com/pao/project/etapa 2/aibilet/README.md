...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Persistenta-JDBC-blue?style=flat-square" alt="JDBC"/>
...
Datele principale ale aplicației sunt persistate într-o bază de date relațională Oracle, accesată prin JDBC. Singurul fișier CSV păstrat este `audit.csv`, folosit exclusiv pentru jurnalizarea acțiunilor.
...
└── src/com/pao/proiect/aibilet/
    ...
    │   ├── TipBiletEvenimentRepository.java
    │   ├── LocEvenimentRepository.java
    │   └── AgentEventAssignmentRepository.java
    │
    ├── config/
    │   ├── DatabaseConfig.java
...
Persistența datelor principale este realizată exclusiv într-o bază de date relațională Oracle, prin repository-uri JDBC singleton. Serviciile aplicației folosesc repository-urile pentru operațiile de creare, citire, actualizare și ștergere.

Repository-urile implementează interfața generică unică `Repository<T, ID>`, acolo unde modelul permite CRUD clasic. Conectarea se face prin `DatabaseConnection`, folosind proprietățile din `db.properties` și driverul Oracle `lib/ojdbc11.jar`.

Tabelele principale sunt:

| Tabel | Conținut |
|-------|----------|
| `utilizatori` | utilizatorii platformei |
| `locatii` | locațiile disponibile |
| `evenimente` | evenimentele publicate |
| `tipuri_bilete_eveniment` | tipurile de bilete asociate evenimentelor |
| `locuri_eveniment` | locurile asociate evenimentelor cu locuri numerotate |
| `bilete` | biletele emise |
| `comenzi` și `comanda_bilete` | comenzile plasate și biletele din comandă |
| `agent_event_assignments` | asignările agenților la evenimente |

Auditul este singurul mecanism CSV rămas. `AuditService` scrie în `audit.csv`, cu structura:
...
javac -cp "lib/ojdbc11.jar" -d out @sources.txt
```
...
### Observație privind auditul

`audit.csv` este creat automat de `AuditService` la prima scriere, dacă nu există deja.

---

## 11. Conturi de test

Conturile și datele de test sunt definite în baza de date, prin scriptul `database/seed_oracle.sql`. Pentru refacerea mediului local, rulați mai întâi `database/schema_oracle.sql`, apoi `database/seed_oracle.sql`.
